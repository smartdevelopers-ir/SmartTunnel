...
public class ProxyServer implements Runnable{
    ...
    protected ServerAuthenticator auth;
   protected ProxyMessage msg = null;

   protected Socket sock=null,remote_sock=null;
   protected ServerSocket ss=null;
    protected UDPRelayServer relayServer = null;
   protected InputStream in,remote_in;
   protected OutputStream out,remote_out;

   protected int mode;
   protected static final int START_MODE	= 0;
   protected static final int ACCEPT_MODE	= 1;
   protected static final int PIPE_MODE	= 2;
   protected static final int ABORT_MODE	= 3;

   static final int BUF_SIZE	= 8192;

    protected Thread pipe_thread1,pipe_thread2;
    protected long lastReadTime;

  protected static int iddleTimeout	= 180000; //3 minutes
  protected static int acceptTimeout	= 180000; //3 minutes

    protected static CProxy proxy;
   ...
   public void start(int port,int backlog,InetAddress localIP) throws IOException {
      try{
        ss = new ServerSocket(port,backlog,localIP);
          Log.i("TTT",String.format("Starting SOCKS Proxy on: %s:%d",
                  ss.getInetAddress().getHostAddress(), ss.getLocalPort()));
        while(true){
          Socket s = ss.accept();
          Log.i("TTT",String.format("Accepted from: %s:%d", s.getInetAddress().getHostName(), s.getPort()));
      ...
      switch(mode){
        case START_MODE:
         try{
           ...
           Log.i("TTT","Main thread(client->remote)stopped.");
         }
        break;
        case ACCEPT_MODE:
          try{
            ...
            abort();
            Log.i("TTT","Accept thread(remote->client) stopped");
          }
        break;
        case PIPE_MODE:
         try{
            ...
            abort();
            Log.i("TT","Support thread(remote->client) stopped");
         }
        break;
        case ABORT_MODE:
        break;
        default:
         Log.i("TTT",String.format("Unexpected MODE %s", mode));
      }
   }
...
/////////////////
   private void startSession() throws IOException{
     ...
     try{
       ...
       Log.i("TTT","Auth throwed exception:", ioe);
       auth = null;
       return;
     }

     if(auth == null){ //Authentication failed
        Log.i("TTT","Authentication failed");
        return;
     }
   ...
   private void handleException(IOException ioe){
    ...
    protected void onConnect(ProxyMessage msg) throws IOException {
        ...
        try {
            ...
            Log.i("TTT",String.format("Connected to %s:%d", s.getInetAddress(), s.getPort()));
            ...
            Log.e("TTT","Failed connecting to remote socket. Exception: ", sE);
   ...
   private void onBind(ProxyMessage msg) throws IOException{
      ...
      Log.i("TTT",String.format("Trying accept on %s:%d", ss.getInetAddress(), ss.getLocalPort()));
   ...
   private void onUDP(ProxyMessage msg) throws IOException{
      ...
      Log.i("TTT",String.format("Creating UDP relay server for %s:%d", msg.ip, msg.port));
...
//////////////////
   ...
   private void doAccept() throws IOException{
      ...
      Log.i("TTT",String.format("Accepted from %s:%d", s.getInetAddress(), s.getPort()));
   ...
   protected static void log(ProxyMessage msg){
      Log.i("TTT","Request version:"+msg.version+
          "\tCommand: "+command2String(msg.command));
      Log.i("TTT","IP:"+msg.ip +"\tPort:"+msg.port+
