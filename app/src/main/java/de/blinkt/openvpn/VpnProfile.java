...
//            useOpenVPN3 = false;
    ...
    synchronized String[] getExternalCertificates(Context context, int tries) {
        ...
        try {
            ...
            if (caChain.length <= 1 && TextUtils.isEmpty(mCaFilename)) {
                Logger.logInfo(context.getString(R.string.keychain_nocacert));
            } else {
            ...
            if (!TextUtils.isEmpty(mCaFilename)) {
                try {
                    ...
                    Logger.logError("Could not read CA certificate" + e.getLocalizedMessage());
                }
            }
            ...
            e.printStackTrace();
            Logger.logError(String.format("KeyChain Access error: %s", e.getLocalizedMessage()));
            if (Build.VERSION.SDK_INT == Build.VERSION_CODES.JELLY_BEAN) {
                if (!mAlias.matches("^[a-zA-Z0-9]$")) {
                    Logger.logError("Some versions of Android 4.1 have problems if the name of the keystore certificate contains non alphanumeric characters (like spaces, underscores or dashes). Try to reimport the certificate without special characters");
                }
            }
            return null;
            ...
            if (tries == 0)
                return null;
            Logger.logError(String.format("Failure getting Keystore Keys (%s), retrying", e.getLocalizedMessage()));
            try {
                Thread.sleep(3000);
    ...
    private byte[] getKeyChainSignedData(byte[] data, OpenVPNManagement.SignaturePadding padding, String saltlen, String hashalg, boolean needDigest) {
        ...
        (NoSuchAlgorithmException | InvalidKeyException | IllegalBlockSizeException | BadPaddingException | NoSuchPaddingException | SignatureException | InvalidAlgorithmParameterException
                        e) {
            Logger.logError(String.format("Error signing with Android keystore key %1$s: %2$s", e.getClass().toString(), e.getLocalizedMessage()));
            return null;
        }
    }
