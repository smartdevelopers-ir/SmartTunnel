...
//    private OpenVPNService mOpenVPNService;
    ...
    private OpenVpnConfig mOpenVpnConfig;
    ...
    private OrbotHelper.StatusCallback statusCallback = new OrbotHelper.StatusCallback() {
        ...
        public void onStatus(Intent statusIntent) {
            ...
            Logger.logDebug("Got Orbot status: " + extras);
        }

        @Override
        public void onNotYetInstalled() {
            Logger.logDebug("Orbot not yet installed");
        }
        ...
        public void onDisabled(Intent intent) {
            Logger.logWarning("Orbot integration for external applications is disabled. Waiting %ds before connecting to the default port. Enable external app integration in Orbot or use Socks v5 config instead of Orbot to avoid this delay.");
        }
    };
        ...
        try {
            ...
            while (true) {
                ...
                try {
                    ...
                    Logger.logException("Error reading fds from socket", e);
                }
                if (fds != null) {
...
//            if (!result)
//                Logger.logWarning("Could not protect VPN socket");
//
//            fdClose(fd);
//
//            return;
...
//            Logger.logException("Failed to retrieve fd from socket (" + fd + ")", e);
//        }
    ...
    private void fdClose(FileDescriptor fd) {
        try {
            ...
            Logger.logException("Failed to close fd (" + fd + ")", e);
        }
    }
        ...
        if (command.startsWith(">") && command.contains(":")) {
            ...
            switch (cmd) {
                ...
                case "INFOMSG":
                    ...
                    break;
                default:
                    Logger.logWarning("MGMT: Got unrecognized command" + command);
            ...
            Logger.logWarning("MGMT: Got unrecognized line from management:" + command);
        }
    }
...
//            mOpenVPNService.trigger_sso(info);
        ...
        {
            Logger.logDebug("Info message from server:" + info);
        }
    }
    ...
    private void processLogMessage(String argument) {
        ...
        Logger.logInfo("ovpn level = " + args[1] + " msg = " +msg);
    }

    boolean shouldBeRunning() {
        return mOpenVpnConfig.isConnected();
    }
    ...
    private void handleHold(String argument) {
        ...
        if (shouldBeRunning()) {
            if (waittime > 1)
                Logger.logMessage(new LogItem(String.format("[OpenVPN] waiting for %d second to retry",waittime)));
            mResumeHandler.postDelayed(mResumeHoldRunnable, waittime * 1000);
//            if (waittime > 5)
//                Logger.logInfo(R.string.state_waitconnectretry, String.valueOf(waittime));
//            else
//                Logger.logDebug(R.string.state_waitconnectretry, String.valueOf(waittime));

        } else {
//            Logger.updateStatePause(lastPauseReason);
        }
    }
    ...
    private void processProxyCMD(String argument) {
        ...
        if (mProfile != null && mProfile.mConnections != null && mProfile.mConnections.length > connectionEntryNumber) {
            ...
            Logger.logError(String.format(Locale.ENGLISH, "OpenVPN is asking for a proxy of an unknown connection entry (%d)", connectionEntryNumber));
        }
        ...
        if (args.length >= 2 && proxyType == Connection.ProxyType.HTTP) {
            ...
            if (proto.equals("UDP")) {
                proxyname = null;
                Logger.logInfo("Not using an HTTP proxy since the connection uses UDP");
            }
        }

        if (proxyType == Connection.ProxyType.ORBOT) {
//            Logger.updateStateString("WAIT_ORBOT", "Waiting for Orbot to start", R.string.state_waitorbot, ConnectionStatus.LEVEL_CONNECTING_NO_SERVER_REPLY_YET);
            OrbotHelper orbotHelper = OrbotHelper.get();
            if (!OrbotHelper.checkTorReceier(mWeakContext.get()))
                Logger.logError("Orbot does not seem to be installed!");

            mResumeHandler.postDelayed(orbotStatusTimeOutRunnable, ORBOT_TIMEOUT_MS);
            orbotHelper.addStatusCallback(mWeakContext.get(), statusCallback);
...
//            Logger.logInfo(R.string.using_proxy, proxyname, proxyname);
    ...
    private void processState(String argument) {
        ...
        String[] reconnectting = {"RECONNECTING", "WAITING"};
        String[] args = argument.split(",",8);
        String currentstate = args[1];


        if (!TextUtils.isEmpty(args[2])){
            Logger.logMessage(new LogItem("[OpenVPN] "+currentstate + " : " + args[2]));
        }else {
            Logger.logMessage(new LogItem("[OpenVPN] "+currentstate));
        }
        for (String s : connected){
            if (Objects.equals(s,currentstate)){
                mOpenVpnConfig.onOpenVpnConnected();
                return;
            }
        }
        for (String s : notconnected){
            if (Objects.equals(s,currentstate)){
                mOpenVpnConfig.onOpenVpnDisconnected();
                return;
            }
        }
        for (String s : reconnectting){
            if (Objects.equals(s,currentstate)){
                mOpenVpnConfig.onOpenVpnReconnecting();
                return;
            }
        }
    }
...
//                mOpenVPNService.setDomain(extra);
            ...
            */
                ...
                if (routeparts.length == 5) {
                    ...
                    mOpenVpnConfig.addRoute(routeparts[0], routeparts[1], routeparts[2], routeparts[4]);
                } else if (routeparts.length >= 3) {
                    mOpenVpnConfig.addRoute(routeparts[0], routeparts[1], routeparts[2], null);
                } else {
            ...
            case "ROUTE6": {
                String[] routeparts = extra.split(" ");
                mOpenVpnConfig.addRoutev6(routeparts[0], routeparts[1]);
                break;
            }
            case "IFCONFIG":
                String[] ifconfigparts = extra.split(" ");
                int mtu = Integer.parseInt(ifconfigparts[2]);
                mOpenVpnConfig.setLocalIP(ifconfigparts[0], ifconfigparts[1], mtu, ifconfigparts[3]);
                break;
            case "IFCONFIG6":
                String[] ifconfig6parts = extra.split(" ");
                mtu = Integer.parseInt(ifconfig6parts[1]);
                mOpenVpnConfig.setMtu(mtu);
                mOpenVpnConfig.setLocalIPv6(ifconfig6parts[0]);
                break;
            case "PERSIST_TUN_ACTION":
                // check if tun cfg stayed the same
                status = mOpenVpnConfig.getTunReopenStatus();
                break;
            case "OPENTUN":
...
//                    mOpenVPNService.addHttpProxy(httpproxy[0], Integer.parseInt(httpproxy[1]));
                    ...
                    Logger.logError("Unrecognized HTTPPROXY cmd: " + Arrays.toString(httpproxy) + " | " + argument);
                }
                break;
            default:
    ...
    private boolean sendTunFD(String needed, String extra) {
        if (!extra.equals("tun")) {
            // We only support tun
            Logger.logError(String.format("Device type %s requested, but only tun is possible with the Android API, sorry!", extra));

            return false;
        }
        ParcelFileDescriptor pfd = mOpenVpnConfig.getFileDescriptor();
        if (pfd == null)
            return false;
        ...
        try {
            ...
            Logger.logException("Could not send fd over socket", exp);
        }
    ...
    private void processPWCommand(String argument) {
        ...
        try {
            ...
            Logger.logError("Could not parse management Password command: " + argument);
            return;
        }
        ...
        if (pw != null) {
            ...
            mOpenVpnConfig.requestInputFromUser(R.string.password, needed);
            Logger.logError(String.format("Openvpn requires Authentication type '%s' but no password/key information available", needed));
        }

    }

    private void proccessPWFailed(String needed, String args) {
        Logger.logError("AUTH_FAILED " +  needed + args + ConnectionStatus.LEVEL_AUTH_FAILED);
    }
    ...
    public void signalusr1() {
        ...
        if (!mWaitingForRelease)
            ...
            Logger.logWarning("state paused reason =" + lastPauseReason);
        }
    }
    ...
    private void processSignCommand(String argument) {
        ...
        String signed_string = mProfile.getSignedData(mWeakContext.get(), arguments[0], padding, saltlen, hashalg, needsDigest);
